...
-module(sutil_bcoding).
-author("Jesper Louis Andersen <jesper.louis.andersen@gmail.com>").

%% API
% Encoding and parsing
-export([encode/1, decode/1, decode_extra/1, decode_dict_item/1, parse_file/1]).
...
%% @end
-spec encode(etorrent_spider_types:bcode()) -> iolist().
...
%% @doc For scrape parsing
%% @end
decode_dict_item(Bin) ->
    case decode_b(Bin) of
        End={end_of_data, _Rest} -> End;
        {Key, Rest1} ->
